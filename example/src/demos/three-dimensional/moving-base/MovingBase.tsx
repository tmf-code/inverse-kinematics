...
import { MathUtils, QuaternionO, Solve3D, V2O, V3, V3O } from 'inverse-kinematics'
...
function useCircularMotion(modelRef: React.MutableRefObject<THREE.Group | undefined>) {
  const angle = useRef(0)
  useFrame((_context, deltaTime) => {
    const firstBone = modelRef.current?.children.find((child) => (child as Bone).isBone) as Bone | undefined
    if (!firstBone) return

    const position = V2O.fromPolar(10, angle.current)
    firstBone.position.x = position[0]
    firstBone.position.z = position[1]

    angle.current += deltaTime / 3
  })
}
